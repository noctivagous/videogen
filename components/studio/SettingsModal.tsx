...
export function SettingsModal() {
  ...
  const setDefaultModel = useStudioStore((s) => s.setDefaultModel);
  ...
  const isCustomDefault = ai.customProviders.some((p) => p.id === ai.defaultProvider);
  const availableModels = getAvailableVideoModels(ai.defaultProvider, isCustomDefault, ai);
  const selectedModelId = getEffectiveModelId(ai);
          ...
          <div className="glass rounded-3xl p-6 border border-surface-700" {...uiSectionProps(UI_SECTIONS.studioSettingsDefaultProvider)}>
            <div className="flex flex-col md:flex-row md:items-center gap-4">
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-400">Bring your own API keys. Generate uses your selected provider and video model.</p>
              </div>
              <div className="w-full md:w-80 space-y-3">
                ...
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-gray-500 block mb-1.5">Video model</label>
                  <select
