...
export function AppsLauncherGrid({
  ...
  return (
    <div className={`apps-launcher-grid grid grid-cols-1 sm:grid-cols-2 gap-3 ${className}`.trim()}>
      {items.map((item) => (
        <button
          key={item.id}
          type="button"
          onClick={() => onSelect(item.id)}
          className={`apps-launcher-card text-left border bg-surface-800 hover:border-brand-500/40 transition-colors ${
        ...
        >
          ...
          <div className="apps-launcher-card__content">
            <div className="apps-launcher-card__title">{item.title}</div>
            <div className="apps-launcher-card__description">{item.description}</div>
          </div>
        </button>
      ))}
    </div>
