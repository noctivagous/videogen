#!/usr/bin/env node
/**
 * Generate model workflow group card JPGs from
 * public/stock/app-styling/model-workflow-groups-prompts.json via xAI.
 *
 * Usage:
 *   npm run stock:model-workflow-groups
 *   npm run stock:model-workflow-groups -- --only=advanced,image-video
 *   npm run stock:model-workflow-groups -- --force
 */
import fs from 'fs';
...
const PROMPTS_PATH = path.join(
  process.cwd(),
  'public/stock/app-styling/model-workflow-groups-prompts.json',
);
...
async function writeGroupCard(buf, destPath) {
...
async function main() {
  ...
  if (!samples.length) {
    throw new Error('No model workflow group samples matched filters');
  }

  const order = topoSort(samples);
  console.log(
    `Model workflow groups: ${order.length} images (dry-run=${dryRun}, force=${force})`,
  );
